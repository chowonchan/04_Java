[객체(objext)]
- 현실 : 식별 가능한 모든것(속성 + 기능을 지님)

- java : 클래스에 정의된 내용대로
          new 연산자에 의해서 Heap메모리에 생성된것

[객체지향 언어 || 객체 지향 프로그래밍(oop)]
- 현실 세계가 객체로 이루어져
  객체간의 상호작용으로 사건이 발생한다
  라는 패러다임을 컴퓨터로 옮겨 놓은것/프로그래밍 방식
...
2) Heap 메모리 영역에 
Nation 클래스에 정의된 내용을 토대로 객체 생성(할당)

  - new 연산자 : Heap 메모리 영역에 새로객체 생성

3) 생성된 객체의 시작 주소를 n2변수에 대입하여 참조할 수 있게 함
...
[Stack] : 호출된 메서드가 쌓이는영역
        + 메서드 내 지역변수도 Stack 영역에 생성

[Heap]  : 동적 메모리 영역, 프로그램 실행 중 객체가 생성 / 소멸
