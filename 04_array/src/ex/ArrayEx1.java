...
public class ArrayEx1 {
	...
	/* 배열(자료구조)
	 * 
	 * - 같은 자료형 변수를 하나의 묶음 으로 다루는 것
	 * 
	 * - 생성된 배열의 요소(묶여진 배열)는
	 * 	 인덱스 생성 번호로 구분된다!(index는 0부터 시작)
	 * 
	 * - 생성된 배열을 활용하기 위해서는 배열을 참조하는
	 * 	 "참조형 변수"를 이용한다!
	 * 
	 * 	[참조형 변수]
	 * - 값x, 주소를 저장하는 변수
	...
	public void method1() {
		...
		arr[0] = 1000;
		arr[1] = 20;
		// arr[2] = 3.14; //int 배열요소에 double 대입 불가!
		arr[2] = 3;
		arr[3] = 500;
...
//		 arr[4] = 999;
	...
	public void method4() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			arr[i] = sc.next().charAt(0); // 입력된 문자열의 0번쨰 문자(char)를 반환
										  // -> char 입력 방법
		}
